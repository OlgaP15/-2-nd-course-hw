...
function startRandomColorGenerator() {
    alert("Запускаем игру 'Генератор случайных цветов'!");
    alert("При каждом клике на кнопку её цвет будет меняться на случайный.");
    ...
    changeColorButton.addEventListener('click', () => {
        ...
        changeColorButton.style.color = getContrastColor(randomColor);
        console.log("Цвет кнопки изменён на:", randomColor);
    });

    stopGameButton.addEventListener('click', () => {
        modal.style.display = 'none';
        alert("Игра 'Генератор случайных цветов' завершена. Спасибо за игру!");
    });
}
